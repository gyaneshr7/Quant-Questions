...
function AllQuestions() {
  ...
  const handleUpdate = async (id, catArray,options) => {
    ...
    const updatedata = await fetch(`http://localhost:8000/question/update/questions/${id}`, {
      method: "PUT",
    ...
    if (removedFirm.length > 0 && firms.length > 0) {
      firmNames.map((firm) => {
        if (removedFirm.includes(firm) && firms.includes(firm)) {
          console.log(firm);
        } else if (removedFirm.includes(firm) && catArray.firmArray.includes(firm)) {
          fdec.push(firm);
    ...
    if (removedDivision.length > 0 && divisions.length > 0) {
      divisionNames.map((div) => {
        if (removedDivision.includes(div) && divisions.includes(div)) {
          console.log(div);
        } else if (removedDivision.includes(div) && catArray.divisionsArray.includes(div)) {
          ddec.push(div);
        } else if (divisions.includes(div) && !catArray.divisionsArray.includes(div)) {
          dinc.push(div);
        }
      })
    } else if (removedDivision.length > 0) {
    ...
    if (removedPosition.length > 0 && positions.length > 0) {
      positionNames.map((pos) => {
        if (removedPosition.includes(pos) && positions.includes(pos)) {
          console.log(pos);
        } else if (removedPosition.includes(pos) && catArray.positionsArray.includes(pos)) {
          pdec.push(pos);
        } else if (positions.includes(pos) && !catArray.positionsArray.includes(pos)) {
          pinc.push(pos);
        }
      })
    } else if (removedPosition.length > 0) {
    ...
    if (removedTag.length > 0 && tags.length > 0) {
      tagNames.map((tag) => {
        if (removedTag.includes(tag) && tags.includes(tag)) {
          console.log(tag);
        } else if (removedTag.includes(tag) && catArray.tagsArray.includes(tag)) {
          tdec.push(tag);
    ...
    <div>
      <div className="dash">
        ...
        <div className="dash-right">
                      ...
                      <div className="ques1-drop">
                        ...
                        <div className="additionals">
                          <FiEdit
                            ...
                            className=""
                          />
                          <MdDelete size="20" onClick={() => { setCategoryData({ firmArray: data.firms, divisionsArray: data.divisions, positionsArray: data.position, tagsArray: data.tags }); handleDelete(data._id) }} />
                        </div>
                      </div>
                    </div>
                    {open && index === id &&
                        ...
                        <div>
                          <div className="detail-ques">
                            ...
                            <div>
                              ...
                              <select name="difficulty" className="opt-font selection" required onChange={(e) => setDifficulty(e.target.value)}>
                                <option value="none" selected disabled hidden>
                                  {data.difficulty}
                                </option>
                            ...
                            <div>
                              ...
                              <select name="answerType" className="opt-font selection" required onChange={(e) => setAnsType(e.target.value)}>
                                <option value="none" selected disabled hidden>
                                  {data.answerType}
                                </option>
                                {categories &&
                                  categories.answerType.map((ans, i) => (
                                    <option key={i} value={ans.name}>{ans.name}</option>
                                  ))
                                }
                              </select>
                            </div>
                            ...
                            }
                            {
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  onChange={(e) => setOption1(e.target.value)}
                                  placeholder={data.options[0]? data.options[0] :''}
                                />
                              </div>
                            }
                            {
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  onChange={(e) => setOption2(e.target.value)}
                                  placeholder={data.options[1]? data.options[1] :""}
                                />
                              </div>
                            }
                            {
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  onChange={(e) => setOption3(e.target.value)}
                                  placeholder={data.options[2]? data.options[2]:""}
                                />
                              </div>
                            }
                            {
                              ...
                              <div>
                                ...
                                <input
                                  type="text"
                                  onChange={(e) => setOption4(e.target.value)}
                                  placeholder={data.options[3]?data.options[3]:""}
                                />
                              </div>
                            }
                            <div>
                            ...
                            <div>
                              ...
                              <div className="form-outline flex-fill mb-0">
                                <Multiselect
                                  ...
                                  onRemove={(name, removedItem) => { setRemovedFirm(arr => [...arr, removedItem]); }}
                                  options={firmNames}
                            ...
                            <div>
                              ...
                              <Multiselect
                                ...
                                isObject={false}
                                onKeyPressFn={function noRefCheck() { }}
                                onRemove={(name, removedItem) => { setRemovedDivision(arr => [...arr, removedItem]); }}
                                onSearch={function noRefCheck() { }}
                            ...
                            <div>
                              ...
                              <Multiselect
                                ...
                                isObject={false}
                                onKeyPressFn={function noRefCheck() { }}
                                onRemove={(name, removedItem) => { setRemovedPosition(arr => [...arr, removedItem]); }}
                                onSearch={function noRefCheck() { }}
                            ...
                            <div>
                              ...
                              <Multiselect
                                ...
                                isObject={false}
                                onKeyPressFn={function noRefCheck() { }}
                                onRemove={(name, removedItem) => { setRemovedTag(arr => [...arr, removedItem]); }}
                                onSearch={function noRefCheck() { }}
                          ...
                          <div className="add-ons">
                            <button className="add-btn-save" onClick={() => handleUpdate(data._id, { firmArray: data.firms, divisionsArray: data.divisions, positionsArray: data.position, tagsArray: data.tags },data.options)}>Save</button>
                            <button
          ...
          {deletebtn && <Modal
            show={deletebtn} onHide={() => setDeleteBtn(false)}
          >
            ...
            <Modal.Footer>
              ...
              <Button variant="light" onClick={() => setDeleteBtn(false)}>Cancel</Button>
            </Modal.Footer>
          </Modal>}
        </div>
      </div>
